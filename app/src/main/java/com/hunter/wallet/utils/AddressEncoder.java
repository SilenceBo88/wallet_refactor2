...
public class AddressEncoder {
    ...
    //解析比特币二维码格式
    public static AddressEncoder decodeBtc(String s) throws IOException {
        String addr = "";
        if (s.startsWith("bitcoin:")){
            addr = s.substring(8);
        }
        return new AddressEncoder(addr);
    }

    //比特币二维码格式加密
    public static String encodeBtc(AddressEncoder a){
        String re = "bitcoin:" + a.address;
        return re;
    }
